...
fn parse_ncch(cia: &mut CiaReader, offs: u64, mut titleid: [u8; 8], icons: bool) {
    ...
    let output_name = format!(
        ...
        base,
        if cia.from_ncsd {
            NCSD_PARTITIONS[cia.cidx as usize].to_string()
        } else {
            cia.cidx.to_string()
        },
        cia.content_id
    ...
    let output_path: PathBuf = parent_dir.join(output_name);

    println!("Creating NCCH: {:?}", output_path);

    let mut ncch: File = File::create(&output_path).unwrap();
